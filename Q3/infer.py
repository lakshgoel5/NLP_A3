...
class Retriever:
    ...
    def build_faiss_index(self, embed_model):
        ...
        index_path = "demo_pool_faiss_index.bin"
        if os.path.exists(index_path):
            print(f"[retriever] loading cached FAISS index from {index_path}...")
            self.index = faiss.read_index(index_path)
            print(f"[retriever] index loaded: {self.index.ntotal} vectors")
            return
        ...
        # Save to disk to reuse in subsequent language runs
        faiss.write_index(self.index, index_path)
        print(f"[retriever] index saved to {index_path} for future runs")
